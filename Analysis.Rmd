...
propsTable$Number <- as.factor(propsTable$Number)
...
expected_prop <- (1 / 16)
z_critical <- qnorm(c(.025,.975))  # Crit valyes for 95% CI, approximately 1.96 by 69-95-99.5 rule

# Loop through each column
for (col in columns_to_plot) {
  col_mean <- mean(propsTable[[col]])
  n <- sum(propsTable[[col]])
  SE <- sqrt((expected_prop * (1 - expected_prop)) / n)
  
  lower_bound <- (expected_prop + z_critical[1] * SE)
  upper_bound <- (expected_prop + z_critical[2] * SE)
  
  
  p <- ggplot(data = propsTable, aes(x=Number, y = (!!sym(col)) / n)) +
    geom_bar(stat="Identity") +
    geom_hline(yintercept = expected_prop, color = "dodgerblue3", linetype = "solid") + # Expected Mean 
    # geom_hline(yintercept = col_mean, color = "red", linetype = "dotted") + # Data Mean
    geom_hline(yintercept = lower_bound, color = "firebrick3", linetype = "dashed") + #
    geom_hline(yintercept = upper_bound, color = "firebrick3", linetype = "dashed") +
    labs(title = col)  # Add title for each plot
  
  # Print the plot
  print(p)
  
  #TODO: prop.test for each number for each col in col_to_plot, then print p-value
}
...
#TODO: IQR range like stat 121 with bar charts for props
